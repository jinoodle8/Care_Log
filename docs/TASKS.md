...
> M6(실모델)과 독립적이므로 학습 데이터를 기다리는 동안 병행한다.
> 어르신 모드는 "버튼 최소화" 원칙(CLAUDE.md 5장)을 지키기 위해, 기록 조회만 홈에 노출하고
> 설정은 작은 진입점으로 둔다.

- [x] **M7-01** 서버 `PATCH /users/me` — 이름·전화번호 수정, 전화번호 중복 시 `PHONE_ALREADY_REGISTERED`
  `[검증]` e2e: 본인 수정 성공, 타인 번호로 변경 시 409/400, 토큰 없이 401
  → 보낸 항목만 수정. 자기 번호를 그대로 보내는 것은 허용하고, 빈 본문은 `NOTHING_TO_UPDATE`로 거부.
- [x] **M7-02** 서버 `POST /users/me/password` — 현재 비밀번호 확인 후 변경
  `[검증]` e2e: 올바른 현재 비밀번호로 변경 후 새 비밀번호로 로그인 성공, 틀린 현재 비밀번호는 거부
  → 어르신 계정은 비밀번호가 없어 `PASSWORD_NOT_SET`. 감사 로그에 비밀번호 값이 남지 않는 것까지 e2e로 확인.
- [x] **M7-03** 서버 `PATCH /users/elders/:id` — 보호자가 연동된 어르신 정보를 대신 수정
  `[검증]` e2e: 연동된 보호자만 성공, 미연동 보호자는 `NOT_LINKED_ELDER` 403
  → 보호자 역할 + 연동 관계를 모두 확인. 어르신 토큰은 `FORBIDDEN`.
- [ ] **M7-04** 알림 설정 — `NotificationPreference` 스키마(3종 on/off) + `GET/PATCH /users/me/notification-preferences`, `PushService`가 설정을 존중
  `[검증]` e2e: 특정 종류를 끄면 해당 푸시가 발송 대상에서 제외되는지 확인
- [ ] **M7-05** 서버 `DELETE /users/me` — 계정 삭제. 마지막 보호자면 어르신 데이터까지 정리, 감사 로그 기록
  `[검증]` e2e: 보호자 삭제 후 연동/기록 처리 규칙 확인, 다른 보호자가 남아 있으면 어르신 데이터 유지
- [ ] **M7-06** 모바일 보호자 설정 화면 — 내 정보 / 비밀번호 / 어르신 관리 / 알림 / 로그아웃 / 계정 삭제
  `[검증]` 실기기 또는 웹에서 각 항목 저장 후 서버 반영 확인
- [x] **M7-07** 모바일 어르신 복약 기록 화면(PRD 4.2.6) — 최근 7일, 큰 글씨 한 줄 요약
  `[검증]` 어르신 토큰으로 본인 기록만 조회되는지 확인, 진단성 문구 없음
  → 날짜별 묶음·오늘/어제 표기·부드러운 판정 문구를 순수 함수로 분리해 10건 테스트. 홈에는 "약 먹기"보다 작은 보조 버튼으로만 노출. **실기기 화면 확인은 미검증.**
- [ ] **M7-08** 모바일 어르신 설정 화면(PRD 4.2.7) — 내 정보·보호자 확인, 알림 권한 상태
  `[검증]` 파괴적 동작이 노출되지 않는지 확인
...
| 2026-08-10 | M5-04·M5-05·M5-06 | `04d7a12` |
| 2026-08-13 | M7-01·M7-02·M7-03 | `8293e19` |
| 2026-08-13 | M7-07 | `43d54de` |
