...
**M1 완료.** M2는 Mock 인식 엔진(M2-01~M2-03), 어르신 촬영 플로우 전체(M2-04~M2-08: 홈→카운트다운→녹화→분석→결과), 서버 logs API(M2-09~M2-10), 모바일 로그 업로드 연동(M2-11)까지 완료. 재개 시 M2-12(서버 `auth` 모듈 — signup/login, JWT 발급)부터 시작한다.
