...
**M1 완료.** M2는 Mock 인식 엔진(M2-01~M2-03), 어르신 촬영 플로우 전체(M2-04~M2-08: 홈→카운트다운→녹화→분석→결과), 서버 logs API(M2-09~M2-10), 모바일 로그 업로드 연동(M2-11), 서버 인증·연동(M2-12~M2-13: auth/links)까지 완료. 재개 시 M2-14(모바일 온보딩 로직 연결 — 회원가입/초대코드 화면을 실제 API에 연결)부터 시작한다.
