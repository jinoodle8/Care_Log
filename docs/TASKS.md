...
- [ ] **M7-01** 서버 `PATCH /users/me` — 이름·전화번호 수정, 전화번호 중복 시 `PHONE_ALREADY_REGISTERED`
  `[검증]` e2e: 본인 수정 성공, 타인 번호로 변경 시 409/400, 토큰 없이 401
- [ ] **M7-02** 서버 `POST /users/me/password` — 현재 비밀번호 확인 후 변경
  `[검증]` e2e: 올바른 현재 비밀번호로 변경 후 새 비밀번호로 로그인 성공, 틀린 현재 비밀번호는 거부
- [ ] **M7-03** 서버 `PATCH /users/elders/:id` — 보호자가 연동된 어르신 정보를 대신 수정
